...
- **Step-up**: JWT WALLT già autenticato → riverifica identità recente → `step_up_token` (JWT, 5 min, `type: step_up`) → header `X-Step-Up-Token` per operazioni sensibili (`reset-account`, `delete-account`, `esporta`). Un unico stato attuale, valido per ENTRAMBI i tipi di utente:
  - **Utenti locali**: `POST /api/auth/verify-password` → `bcrypt.compare` sulla password reale.
  - **Utenti Google OAuth** (senza password): `POST /api/auth/google/challenge` (genera nonce/challenge legato a `req.userId`, 2 minuti) → pulsante ufficiale "Continua con Google" (Google Identity Services) → `POST /api/auth/verify-google { credential, challenge }`, che verifica crittograficamente l'ID token con `google-auth-library` (firma, audience, issuer, scadenza), il nonce, la freschezza, il single-use del challenge, e `payload.sub === user.google_id`. **Vera ri-autenticazione**, non una frase pubblica.
  - Le stringhe `CONFERMA`/`ELIMINA`/`RESETTA` sono **solo conferme testuali UX** nei modali (validate lato body da `resetAccount`/`deleteAccount`), indipendenti dallo step-up: digitarle da sole non produce mai un `step_up_token`.
- Vedi `docs/SECURITY.md` per l'analisi completa (proprietà garantite, limiti noti) e `docs/DECISIONS.md` per la storia di questa decisione (implementata → rimossa → reimplementata).
...
Comunicazione: Axios con `baseURL = VITE_API_URL` (default `http://localhost:3000/api`), header `Authorization: Bearer <token>`.
...
| **Migrazioni DB** | Auto-run all'avvio SOLO fuori produzione (disabilitato quando `NODE_ENV=production`, vedi `RUN_MIGRATIONS_ON_BOOT`); 16 file con possibili duplicati | `server.js`, `migrations/` |
...
7. Run available validation: `cd server && npm test`.
8. Review the diff.
