...
106 endpoint REST sotto `/api/*` (incluse le 3 rotte `/api/fondo-emergenza`, le 7 rotte `/api/categorie`, le due dello step-up Google — `POST /api/auth/google/challenge` e `POST /api/auth/verify-google` — e le 6 di Piano Smart). Vedi `docs/API.md` per inventario completo e `docs/piano-smart-api-contract.md` per il contratto di Piano Smart.
