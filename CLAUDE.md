...
- Migrazioni via `sequelize-cli` (41 file in `server/migrations/`)
...
11. **Spese ricorrenti**: cron giornaliero 09:00 Europe/Rome, ma **solo frequenza `mensile`** è processata. **Spese programmate**: `ricorrente_frequenza: 'una_tantum'` + `ricorrente_data` è un caso a parte, un addebito unico a data fissa (non periodico). Il cron la addebita quando `oggi >= ricorrente_data`, non solo quel giorno esatto: se il cron salta un giorno (deploy, downtime) recupera al passaggio successivo invece di perdere l'occorrenza. Nella stessa transazione dell'addebito chiude il promemoria a `stato_ricorrenza: 'terminata'`, così smette di comparire fra gli impegni — un promemoria rimasto `attiva` dopo l'addebito bloccherebbe due volte lo stesso euro. Fino a quando non è addebitata pesa sul saldo effettivo (Regola 20) se cade entro 30 giorni da oggi, **oppure è già passata**: una data scaduta e mai addebitata è l'impegno più certo che esista, non va esclusa dall'orizzonte. **`muoveSaldo(movimento)`** (`ricorrenti.service.js`) è il predicato che decide se una riga muove davvero denaro: falso solo per una spesa programmata non ancora eseguita, vero per tutto il resto. Una spesa programmata è una PROMESSA, non un movimento avvenuto — crearla NON scala il conto, lo fa solo il cron quando genera la sua occorrenza. Lo leggono sia chi scrive i saldi (`movimenti.controller.js`: create/update/delete) sia chi li riepiloga (`conti.controller.js`, per la variazione mensile del patrimonio): se questi punti divergessero da `muoveSaldo`, la stessa spesa verrebbe contata un numero di volte diverso a seconda di chi la guarda (è successo: creare una programmata scalava subito il conto, il cron la riaddebitava alla sua data, e il saldo effettivo la sottraeva una terza volta come impegno — corretto nel commit `d20cf0b`). In `updateMovimento` il predicato entra nel calcolo dell'impatto vecchio e nuovo, non nel tipo/importo grezzo: convertire un movimento da/verso `una_tantum` deve restare bilanciato, e una promessa non può far fallire l'aggiornamento per saldo insufficiente.
...
|---|---|---|
| **Gestione saldo** | Ogni movimento modifica `Conto.saldo` in transazione, **tranne** una spesa programmata non ancora addebitata (`muoveSaldo`, Regola 11): create/update/delete e il riepilogo mensile devono leggere lo stesso predicato o la stessa uscita viene contata un numero di volte diverso a seconda del punto | `movimenti.controller.js`, `conti.controller.js`, `services/ricorrenti.service.js` (`muoveSaldo`) |
...
| **Cron ricorrenti** | Crea movimenti automaticamente ogni giorno; per una spesa programmata (`una_tantum`) l'addebito e la chiusura del promemoria (`stato_ricorrenza: 'terminata'`) devono stare nella stessa transazione, altrimenti un crash a metà lascia un impegno fantasma che blocca liquidità già spesa | `ricorrenti.service.js` |
| **Migrazioni DB** | Auto-run all'avvio SOLO fuori produzione (disabilitato quando `NODE_ENV=production`, vedi `RUN_MIGRATIONS_ON_BOOT`); 41 file con possibili duplicati. Su Supabase si lanciano a mano con `npm run migrate:production` (`NODE_ENV=migration` + `DATABASE_MIGRATION_URL`) | `server.js`, `migrations/` |
